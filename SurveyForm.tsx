...
export interface SurveyData {
  ...
  q6: string; // Reflection on AI
  q7: string; // Honest Opinion (New)
}
...
export const QUESTIONS = {
  ...
  q6: "Describe a specific moment where you chose to ignore or modify the AI's advice because you felt your own judgment was better.",
  q7: "Please share your honest opinion on the activity and the app. What worked well for you? What was frustrating or unhelpful?"
};

const SurveyForm: React.FC<SurveyFormProps> = ({ onSubmit }) => {
  const [data, setData] = useState<SurveyData>({
    q1: 0, q2: 0, q3: 0, q4: 0, q5: 0, q6: '', q7: ''
  });

  // Check if all Likert scales are > 0 and text areas are not empty
  const isValid = 
    data.q1 > 0 && 
    data.q2 > 0 && 
    data.q3 > 0 && 
    data.q4 > 0 && 
    data.q5 > 0 && 
    data.q6.trim().length > 0 && 
    data.q7.trim().length > 0;
    ...
    <div className="max-w-3xl mx-auto p-6 animate-[fadeIn_0.5s_ease-out]">
      <div className="bg-gray-800 rounded-xl shadow-2xl border border-gray-700 overflow-hidden">
        ...
        <div className="p-8 space-y-8">
          ...
          {/* Likert Scale Questions (Q1-Q5) */}
          {[
            { key: 'q1', text: QUESTIONS.q1 },
            { key: 'q2', text: QUESTIONS.q2 },
            { key: 'q3', text: QUESTIONS.q3 },
            { key: 'q4', text: QUESTIONS.q4 },
            { key: 'q5', text: QUESTIONS.q5 },
          ].map((item) => (
                ...
                {[1, 2, 3, 4, 5].map((num) => (
                  <label key={num} className="flex flex-col items-center cursor-pointer group">
                    <input 
                      ...
                      name={item.key} 
                      value={num} 
                      checked={data[item.key as keyof SurveyData] === num}
                      onChange={() => handleLikertChange(item.key as keyof SurveyData, num)}
                      className="hidden" 
                    />
                    <div className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full border-2 transition-all font-bold text-lg
                      ${data[item.key as keyof SurveyData] === num 
          ...
          <div className="space-y-3">
            ...
            <textarea 
              ...
              className="w-full bg-gray-900 border border-gray-600 rounded-lg p-4 text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent min-h-[100px]"
            />
          </div>

          {/* Open Ended Question Q7 (New) */}
          <div className="space-y-3 pt-4 border-t border-gray-700">
            ...
            <textarea 
              value={data.q7}
              onChange={(e) => setData(prev => ({ ...prev, q7: e.target.value }))}
              placeholder="Honest feedback on the app functionality or the learning activity..."
              className="w-full bg-gray-900 border border-gray-600 rounded-lg p-4 text-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent min-h-[100px]"
            />
          </div>
